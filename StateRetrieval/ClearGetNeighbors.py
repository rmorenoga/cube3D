...
def clear_neighbors_flag(start_time,file_name,server_IP):
    global connected
    requestTime = datetime.now()
    data = (asyncio.run(concurrent_get(uri="clear_get_neighbors",server_IP=server_IP)))
...
def clearNeighborsFlag(expected=26,initial_connection_attempts=10,file_name="clearNeighborsResponses"+str(time.time())+".csv",server_IP=21):

    start_time = time.time()

    attempt_to_connect(initial_connection_attempts,expected,start_time=start_time,server_IP=server_IP)

    clear_neighbors_flag(start_time,file_name,server_IP=server_IP)


    print("Finished changing all neighbor modules mode")



def main():
    clearNeighborsFlag(expected=33,initial_connection_attempts=10,file_name="clearNeighborsResponses"+str(time.time())+".csv",server_IP=22)

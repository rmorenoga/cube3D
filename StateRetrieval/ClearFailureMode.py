...
def attempt_to_connect(attempts,expected,start_time,server_IP = 21):
...
def clear_fail_flag(start_time,file_name, server_IP):
    global connected
    requestTime = datetime.now()
    data = (asyncio.run(concurrent_get(uri="clear_fail", server_IP=server_IP)))
...
def clearFailFlag(expected=26,initial_connection_attempts=10,file_name="clearFailResponses"+str(time.time())+".csv",server_IP=21):

    start_time = time.time()

    attempt_to_connect(initial_connection_attempts,expected,start_time=start_time,server_IP=server_IP)

    clear_fail_flag(start_time,file_name,server_IP=server_IP)
...
def main():
    clearFailFlag(expected=33,initial_connection_attempts=10,file_name="clearFailResponses"+str(time.time())+".csv",server_IP=84)

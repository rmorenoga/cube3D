...
def clear_damage_flag(start_time,file_name,server_IP):
    global connected
    requestTime = datetime.now()
    data = (asyncio.run(concurrent_get(uri="clear_damage",server_IP=server_IP)))
    ...
    if len(connected)>0:
        ...
        for response in data:
            if type(response)==dict:
                row = [experimentTime,requestTime,responseTime,response["fail_state"],response["macId"],response["damage_mode"]]
                dataTable.append(row)
            else:
...
def clearDamageFlag(expected=26,initial_connection_attempts=10,file_name="clearDamageResponses"+str(time.time())+".csv",server_IP=21):

    start_time = time.time()

    attempt_to_connect(initial_connection_attempts,expected,start_time=start_time,server_IP=server_IP)

    clear_damage_flag(start_time,file_name,server_IP=server_IP)


    print("Finished clearing damage mode in all modules")

def main():
    clearDamageFlag(expected=33,initial_connection_attempts=10,file_name="clearDamageResponses"+str(time.time())+".csv",server_IP=84)
